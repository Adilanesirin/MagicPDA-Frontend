...
export default function SalesReturnBarcodeEntry() {
 ...
 useEffect(() => {
  const initialize = async () => {
    ...
    // Restore cart from DB so items survive navigation
    ...
    const enriched = saved.map(item => ({
      ...
      userid: userId,
      supplier_code: "",
      itemcode: item.code || item.barcode,
      sale_date: today,
    }));
 ...
 const loadAllProducts = async () => {
  try {
    const info = await db.getAllAsync(`PRAGMA table_info(product_data)`);
    console.log("📋 product_data columns:", JSON.stringify(info, null, 2));
...
const handleAddAllToCart = async () => {
  ...
  setCartItems(enrichedItems);   // replace, not append — avoids duplicates on re-open
                ...
                <View key={`${item.barcode}-${index}-${item.scannedAt}`} style={[styles.productCard, getCardStyle(item, index)]}>
                  ...
                  <View style={styles.productDetails}>
                   ...
                   <View style={styles.detailRow}>
                      ...
                      <Text style={styles.detailText}>S.Price: <Text style={styles.mrpText}>₹{item.S1 || 0}</Text></Text>
                    </View>
